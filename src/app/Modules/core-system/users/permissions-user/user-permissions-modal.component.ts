...
import {
  ...
  UpdateUserPermissionsInputDto,
  UserDto,
  UserFlatPermissionDto,
  UserPermissionTreeDto,
  UserServiceProxy,
...
@Component({
  selector: 'app-users-permissions-modal',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
...
export class UsersPermissionsModalComponent implements OnInit, AfterViewInit {
  ...
  readonly selectedModuleKey = signal('');
  ...
  load(): void {
    if (!this.userId || this.userId <= 0) {
      this.error.set('No se recibió un userId válido para cargar permisos.');
      this.user.set(null);
    ...
    forkJoin({
      ...
      permissionsForEdit: this.userService.getUserPermissionsForEdit(this.userId),
    })
      .pipe(finalize(() => this.loading.set(false)))
      .subscribe({
        next: ({ user, permissionsForEdit }) => {
          this.user.set(user);

          if (!this.userName?.trim()) {
            this.userName = this.buildUserName(user);
          }

          const permissions = this.buildPermissionStore(permissionsForEdit);
          this.allPermissions.set(permissions);

          this.rebuildModules(permissionsForEdit);
        ...
        error: (e) => {
          ...
          this.rowViewportMap.set(new Map<string, RowViewportState>());
          this.lastHiddenParentTitle.set('');
          this.error.set(e?.message ?? 'Error cargando permisos del usuario');
        },
      });
  }
  ...
  saveAll(): void {
    if (!this.userId || this.userId <= 0) {
      this.error.set('No fue posible identificar el usuario a actualizar.');
      return;
    }
    ...
    const input = new UpdateUserPermissionsInputDto();
    input.id = this.userId;
    input.grantedPermissionNames = (this.allPermissions() ?? [])
  ...
  private rebuildModules(sourceData?: GetUserPermissionsForEditOutput): void {
  ...
  private buildModulesFromTree(tree: UserPermissionTreeDto[]): ModuleVM[] {
    const modules: ModuleVM[] = [];
  ...
  private buildUserName(user: UserDto | null): string {
    if (!user) {
      return '';
    }
    ...
    const joined = `${user.name ?? ''} ${user.surname ?? ''}`.trim();
    return joined || user.userName || '';
  }

  private buildPermissionStore(data: GetUserPermissionsForEditOutput): LocalPermission[] {
  ...
  private flattenFromTree(tree: UserPermissionTreeDto[]): LocalPermission[] {
    const out: LocalPermission[] = [];

    const visit = (node: UserPermissionTreeDto | undefined): void => {
      if (!node?.name) {
        return;
      }
  ...
  private bumpSelection(): void {
    this.selectionTick.update((value) => value + 1);
  }
}