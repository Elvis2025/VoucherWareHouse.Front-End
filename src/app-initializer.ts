...
export class AppInitializer {
                            ...
                            async (result) => {
                                try {
                                    if (this.shouldLoadLocale()) {
                                        ...
                                        await this.loadAngularLocaleData(angularLocale);
                                    }
    ...
    private shouldLoadLocale(): boolean {
        const lang = (abp.localization?.currentLanguage?.name || '').toLowerCase();
        return !!lang && lang !== 'en' && lang !== 'en-us';
    }
    ...
    /**
     * Carga el locale data de Angular sin usar rutas hacia node_modules.
     * Agrega aquí SOLO los locales que uses realmente.
     */
    private async loadAngularLocaleData(angularLocale: string): Promise<void> {
        const normalized = (angularLocale || 'en').toLowerCase();
        ...
        const loaders: Record<string, () => Promise<any>> = {
            ...
            // portugués genérico (evita el error de pt-BR no encontrado)
        ...
        const loader = loaders[normalized] || loaders[base] || loaders['en'];
    ...
    private resolveTenancyName(appBaseUrl: string): string | null {
